...
class PickingExample: public Platform::Application {
    ...
    private:
        ...
        void rotateObjectAroundSelected(std::list<PickableObject*> notSelected, PickableObject* selected, int handedness);
        ...
        Math::Rad<float> rotationDegree = Math::Rad<float>(Math::Deg<float>(10));
};
...
PickingExample::PickingExample(const Arguments& arguments): Platform::Application{arguments, Configuration{}.setTitle("Magnum object picking example")}, _framebuffer{GL::defaultFramebuffer.viewport()} {
    ...
    /* Set up objects */
    (*(_objects[0] = new PickableObject{1, _shader, 0xcd3431_rgbf, _sphere, _scene, _drawables}))
        .translate({-1, -0.3f, -3});
    (*(_objects[1] = new PickableObject{2, _shader, 0xcd3431_rgbf, _sphere, _scene, _drawables}))
        .translate({1, -1.7f, -3});
    (*(_objects[2] = new PickableObject{3, _shader, 0xcd3431_rgbf, _sphere, _scene, _drawables}))
        .translate({1, 1, -3})
        .scale(Vector3(0.75f));
...
void PickingExample::rotateObjectAroundSelected(std::list<PickableObject*> notSelected, PickableObject* selected, int handedness)
{
    Vector3 vec1 = (_cameraObject->absoluteTransformationMatrix() * Vector4(0, 0, 0, 1)).xyz();
    Vector3 vec2 = (selected->absoluteTransformationMatrix() * Vector4(0, 0, 0, 1)).xyz();
    Vector3 rotationAxis = (vec2 - vec1).normalized() * handedness;
    for (auto not_selected : notSelected)
    {
        not_selected->rotate(rotationDegree, rotationAxis);
    }
}

void PickingExample::keyPressEvent(KeyEvent& event)
{
    std::list<PickableObject*> notSelected;
    PickableObject* selected;
    for (PickableObject* object : _objects)
    {
        if (object->getSelected())
            selected = object;
        else
            notSelected.push_back(object);
    }
    if (event.key() == KeyEvent::Key::I)
    {
        rotateObjectAroundSelected(notSelected, selected,1);
    }
